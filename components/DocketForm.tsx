...
export const DocketForm: React.FC<DocketFormProps> = ({ docket, onSave, onDelete, onClose, suppliers, saveSupplier, agents, loading: isSaving, forceReadOnly, readOnlyBanner }) => {
    ...
    const financialSummary = useMemo(() => {
        ...
        const invoicesSubtotal = (formState.invoices && formState.invoices.length > 0)
            ? formState.invoices.reduce((sum, inv) => sum + (inv.subtotal || 0), 0)
            : 0;

        const itineraryGrossTotal = flightsTotal.grossBilled + hotelsTotal.grossBilled + excursionsTotal.grossBilled + transfersTotal.grossBilled + serviceChargeTotals.grossBilled;
        const grandTotalGross = invoicesSubtotal > 0 ? invoicesSubtotal : itineraryGrossTotal;
        const grandTotalNet = flightsTotal.netCost + hotelsTotal.netCost + excursionsTotal.netCost + transfersTotal.netCost + serviceChargeTotals.netCost;
        ...
        return {
            ...
            serviceCharge: { netCost: serviceChargeTotals.netCost, grossBilled: serviceChargeTotals.grossBilled, profit: serviceChargeTotals.grossBilled - serviceChargeTotals.netCost },
            grandTotalGross,
            grandTotalNet,
            grandTotalWithGST,
            totalGST,
    ...
    const summaryItems = { flights: financialSummary.flights, hotels: financialSummary.hotels, excursions: financialSummary.excursions, transfers: financialSummary.transfers, 'service charge': financialSummary.serviceCharge };
    ...
    <>
            ...
            <div className="flex-grow lg:w-2/3 p-4 sm:p-6 overflow-y-auto">
                ...
                {activeTab === 'itinerary' && (<>
                    <div className={`${formState.status === BookingStatus.Confirmed ? 'confirmed-mode' : 'editable-mode'}`}>
                     ...
                     <Section title="Service Charge" icon={Icons.payment} bgClass="bg-[#f0fff9]"><div className="p-4 border rounded-md mb-2 bg-slate-50 grid grid-cols-2 md:grid-cols-4 gap-4 items-end"><FormInput label="Net Cost" type="number" value={formState.itinerary.serviceCharge?.netCost || 0} onChange={e => setFormState(p => ({ ...p, itinerary: { ...p.itinerary, serviceCharge: { ...(p.itinerary.serviceCharge||{ netCost:0, grossBilled:0 }), netCost: +e.target.value } } }))} disabled={isReadOnly} /><FormInput label="Gross Billed" type="number" value={formState.itinerary.serviceCharge?.grossBilled || 0} onChange={e => setFormState(p => ({ ...p, itinerary: { ...p.itinerary, serviceCharge: { ...(p.itinerary.serviceCharge||{ netCost:0, grossBilled:0 }), grossBilled: +e.target.value } } }))} disabled={isReadOnly} /></div></Section>
                     </div>
                 </>)}
        ...
        {invoiceModalOpen && (
            <InvoiceGenerator 
                docket={{ ...(docket ? { id: docket.id } : { id: `TEMP-${Date.now()}` }), ...formState } as any}
